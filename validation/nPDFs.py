...
# The PDF set labels
pdfsetlab=[r"${\rm nNNPDF3.0}$",\
           r"${\rm EPPS21}$"]

q = 14 # GeV
error_option=["mc_68cl","ct"]
filelabel="-neutrinoSFs_q14gev"
...
# Central values (only for EPPS12)
    ...
    for k in range(2*nx):
        ...
        for ifl in range(nfl):
            ...
            if(ifl==4): # u
                if(iset==0):
                    fit1_cv[ifl][k] = pA.xfxQ(+2,x,q)/pP.xfxQ(+2,x,q)                                          
                if(iset==1):
                    fit2_cv[ifl][k] = pA.xfxQ(+2,x,q)/pP.xfxQ(+2,x,q)
            #----------------------------------------------------------------
            if(ifl==5): # d
                if(iset==0):
                    fit1_cv[ifl][k] = pA.xfxQ(+1,x,q)/pP.xfxQ(+1,x,q)                                             
                if(iset==1):
                    fit2_cv[ifl][k] = pA.xfxQ(+1,x,q)/pP.xfxQ(+1,x,q)
            #----------------------------------------------------------------
            if(ifl==6): # ubar
                if(iset==0):
                    fit1_cv[ifl][k] = pA.xfxQ(-2,x,q)/pP.xfxQ(-2,x,q)                                          
        ...
        if(iset==1):
            ...
            for ix in range(2*nx):
                for ifl in range(nfl):
                    ...
                    # Correct 68%CL
                    p2_error[ifl][ix]=p2_error[ifl][ix]/2/1.642
...
labelpdf=[r"$R^{\rm Pb}_{F_2^\nu}(x,Q)$",r"$R^{\rm Pb}_{F_2^{\bar{\nu}}}(x,Q)$",\
          r"$R^{\rm Pb}_{F_2^{\nu+\bar{\nu}}}(x,Q)$",r"$R^{\rm Pb}_{F_2^{\nu-\bar{\nu}}}(x,Q)$",\
          r"$R^{\rm Pb}_{u}(x,Q)$",r"$R^{\rm Pb}_{d}(x,Q)$",\
          r"$R^{\rm Pb}_{\bar{u}}(x,Q)$",r"$R^{\rm Pb}_{\bar{d}}(x,Q)$"]
yranges=[[0.4,1.70],[0.4,1.70],[0.4,1.70],[-1.0,1.0],\
         [0.4,1.70],[0.4,1.70],[0.4,1.70],[0.4,1.7]]
...
for ifl in range(nfl):
    ...
    if(ifl==0):
        ...
        ax.text(0.60,0.10,string,fontsize=16,transform=ax.transAxes)
