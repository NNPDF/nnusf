...
for iset in range(nset):
    ...
    # HERAPDF: symmetric Hessian
        ...
        if(iset==2):
            ...
            for ix in range(2*nx):
                for isf in range(nsf):
                    p3_mid[isf][ix]=fit3_cv[isf][ix] # Central Hessian value
                    p3_error[isf][ix]=0 # initialisation
                    for ieg in range(neig):
                        p3_error[isf][ix] = p3_error[isf][ix] \
                            + (fit3[ieg][isf][ix] - p3_mid[isf][ix] )**2.0
                    p3_error[isf][ix]=math.sqrt(p3_error[isf][ix])
            p3_high = p3_mid + p3_error 
            p3_low = p3_mid - p3_error
...
yranges=[[0,4],[0,4],[-0.5,2.0],[-0.5,2.0]]
labelpdf=[r"$F_2^{\nu p}(x,Q)$",r"$F_2^{\bar{\nu} p}(x,Q)$",\
          r"$xF_3^{\nu p}(x,Q)$",r"$xF_3^{\bar{\nu} p}(x,Q)$"]
...
for isf in range(nsf):
    ...
    ax.text(0.05,0.85,r'$Q=1.65~{\rm GeV}$',fontsize=16,transform=ax.transAxes)
...
py.savefig('StructureFunction-abs'+filelabel+'.pdf')
print('output plot: StructureFunction-abs'+filelabel+'.pdf')
...
py.tight_layout(pad=1, w_pad=1, h_pad=1.0)
py.savefig('StructureFunction-rat'+filelabel+'.pdf')
print('output plot: StructureFunction-rat'+filelabel+'.pdf')
